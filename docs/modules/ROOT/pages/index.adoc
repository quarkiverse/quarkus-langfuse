...
Simply inject `LangfuseApi` into your CDI beans for raw REST API access, or inject `LangfuseOperations` / `AsyncLangfuseOperations` for higher-level operations such as finding resources by name, checking existence, and walking paginated collections (see xref:api-operations.adoc[Higher-Level API Operations]).
...
| Manage evaluation rules (unstable)

| `unstableEvaluators()`
| Manage evaluators (unstable)
