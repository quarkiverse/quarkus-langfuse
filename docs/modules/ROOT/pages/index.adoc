...
* **Higher-level API operations** - Curated `LangfuseOperations` and `AsyncLangfuseOperations` beans over seventeen domains, offering lookup by id or name, existence checks, filtered views, lazy pagination, idempotent initialization and batch deletes that report an outcome per identifier. See xref:api-operations.adoc[Higher-Level API Operations].
...
This version of the extension requires **Langfuse v4**. If you are upgrading from a previous version of the extension that targeted Langfuse v3, complete the https://langfuse.com/changelog/2025-05-14-langfuse-v4[Langfuse v4 migration] before upgrading the extension.

=== OpenAPI spec validation

The Langfuse v4 OpenAPI spec uses the `const` keyword (an OpenAPI 3.1 feature) while declaring version 3.0.1. The `openapi-generator-maven-plugin` cannot validate the spec with this incompatibility, so `skipValidateSpec` is set to `true` in the `langfuse-client` module's POM. This does not affect code generation -- only the upfront validation check is bypassed. This is tracked upstream as https://github.com/OpenAPITools/openapi-generator/issues/10445[openapi-generator#10445].
...
TIP: When <<devservices,DevServices>> is enabled (the default in dev/test mode), `base-url`, `public-key`, and `secret-key` are automatically configured for you. You only need to set these properties when connecting to an external Langfuse instance or in production.

The `public-key` and `secret-key` properties correspond to the Langfuse **project public key** and **project secret key**, respectively. You can find these in the Langfuse dashboard under **Settings > API Keys**.
...
Simply inject `LangfuseApi` into your CDI beans for raw REST API access, or inject `LangfuseOperations` / `AsyncLangfuseOperations` for higher-level operations such as finding resources by id or name, checking existence, restricting a collection with a filter, deleting in batches, and walking paginated collections (see xref:api-operations.adoc[Higher-Level API Operations]).

The extension automatically configures and provides the implementations:
...
When the `quarkus-opentelemetry` extension is present in your project, the Langfuse extension automatically exports OpenTelemetry span data to Langfuse. No additional configuration is needed -- the extension uses your existing `quarkus.langfuse.public-key` and `quarkus.langfuse.secret-key` for authentication.
...
The extension automatically configures the `quarkus.langfuse.base-url`, `quarkus.langfuse.public-key`, and `quarkus.langfuse.secret-key` properties to point to the DevServices instance.
