...
/**
 * Higher-level operations over Langfuse annotation queues, returning Mutiny types.
 *
 * <p>
 * Obtained from {@link AsyncLangfuseOperations#annotationQueues()}. The asynchronous counterpart of
 * {@link AnnotationQueueOperations}; the two behave identically apart from how absence is
 ...
 *
 * @see AnnotationQueueOperations
 */
public sealed interface AsyncAnnotationQueueOperations extends AsyncPagedOperations<AnnotationQueue>
        permits DefaultAsyncAnnotationQueueOperations {

    /**
     * Finds an annotation queue by its id.
     *
     * <p>
     * <strong>Emits {@code null} if no annotation queue has that id.</strong>
     *
     * <p>
     ...
     * @return the matching annotation queue, or {@code null} if no queue has that id
     * @throws IllegalArgumentException if {@code id} is {@code null} or blank
     */
    Uni<AnnotationQueue> findById(String id);

    /**
     * Finds an annotation queue by its exact name.
     *
     * <p>
     * <strong>Emits {@code null} if no annotation queue has that name.</strong>
     *
     * <p>
     ...
     * @return the matching annotation queue, or {@code null} if no queue has that name
     * @throws IllegalArgumentException if {@code queueName} is {@code null} or blank
     */
    Uni<AnnotationQueue> findByName(String queueName);

    /**
     * Whether an annotation queue with the given exact name exists.
     *
     * @param queueName the annotation queue name to look for, must not be {@code null} or blank
     * @return {@code true} if a queue with that name exists. Never {@code null}
     * @throws IllegalArgumentException if {@code queueName} is {@code null} or blank
     */
    default Uni<Boolean> exists(String queueName) {
        return findByName(queueName)
                .map(Objects::nonNull);
    }
     ...
     * @return the existing or newly created annotation queue. Never {@code null}
     */
    Uni<AnnotationQueue> createIfAbsent(CreateAnnotationQueueRequest request);
     ...
     *
     * <p>
     * <strong>The queue id is validated here</strong>, not on first use, so a blank id throws from this
     * call rather than emitting a failure later from a traversal. The queue is not looked up: a view
     * over a queue that does not exist is created happily and reports the absence when it is used.
     *
     * @param queueId the id of the queue whose items to operate on, must not be {@code null} or blank
     * @return the operations over that queue's items. Never {@code null}
     * @throws IllegalArgumentException if {@code queueId} is {@code null} or blank. Thrown from this
     *         call rather than emitted as a failure
     */
    AsyncAnnotationQueueItemOperations items(String queueId);
}
