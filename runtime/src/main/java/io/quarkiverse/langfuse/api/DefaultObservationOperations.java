...
final class DefaultObservationOperations extends AbstractCursorOperations<ObservationV2>
        implements ObservationOperations {
    private final ObservationsApi observationsApi;
    private final LangfuseConfig config;

    DefaultObservationOperations(ObservationsApi observationsApi, LangfuseConfig config) {
    ...
    private DefaultObservationOperations(ObservationsApi observationsApi, LangfuseConfig config,
    ...
    @Override
    public ObservationOperations matching(ObservationFilter filter) {
        return new DefaultObservationOperations(this.observationsApi, this.config,
                ValidationUtils.ensureNotNull(filter, "Filter"));
    }

    private static CursorResult<ObservationV2> fetch(ObservationsApi observationsApi, Cursor cursor,
            ObservationFilter filter) {
        var response = observationsApi.observationsGetMany(request(cursor, filter));

        return CursorResults.from(cursor, response.getData(), response.getMeta(), ObservationsV2Meta::getCursor);
    }
