...
final class DefaultExperimentItemOperations extends AbstractCursorOperations<ExperimentItem>
        implements ExperimentItemOperations {
    ...
    DefaultExperimentItemOperations(ExperimentsApi experimentsApi, LangfuseConfig config,
    ...
    // why neither needs a widened CursorFetcher nor a change to Pagination: a view is just this class
    // constructed over a different fetcher.
    private DefaultExperimentItemOperations(ExperimentsApi experimentsApi, LangfuseConfig config,
    ...
    @Override
    public ExperimentItemOperations matching(ExperimentItemFilter filter) {
        return new DefaultExperimentItemOperations(this.experimentsApi, this.config, this.fromStartTime,
                this.toStartTime, ValidationUtils.ensureNotNull(filter, "Filter"));
    }

    private static CursorResult<ExperimentItem> fetch(ExperimentsApi experimentsApi, Cursor cursor,
            OffsetDateTime fromStartTime, OffsetDateTime toStartTime, ExperimentItemFilter filter) {
        var response = experimentsApi.experimentsListItems(request(cursor, fromStartTime, toStartTime, filter));

        return CursorResults.from(cursor, response.getData(), response.getMeta(), ExperimentsResponseMeta::getCursor);
    }
